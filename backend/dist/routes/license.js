...
router.post('/generate', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const license = await licenseService_1.licenseService.generateLicense(workId, userId, userEmail, {
            authorName,
            dob,
            address,
            mobile,
...
// Get user's licenses
router.get('/my-licenses', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.uid;
        const licenses = await licenseService_1.licenseService.getUserLicenses(userId);
        res.json({ licenses });
    }
    catch (error) {
...
router.get('/download/:licenseId', auth_1.authMiddleware, async (req, res) => {
    try {
        const { licenseId } = req.params;
        const userId = req.user?.uid;
        const license = await licenseService_1.licenseService.verifyLicense(licenseId);
        ...
        const work = await workService_1.workService.getWork(license.workId, userId);
        if (!work) {
            return res.status(404).json({ error: 'Work not found' });
        }
        // Prepare temp write stream
        const tempDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), 'license-'));
        const tempPath = path_1.default.join(tempDir, `license_${license.id}.pdf`);
        ...
        const addHeaderFooter = (doc) => {
            ...
            // Header (two lines)
            doc.font('Courier-Bold').fontSize(12).fillColor('#333').text(headerLine1, margins.left, margins.top + 15, { width: width - margins.left - margins.right, align: 'left', lineBreak: false });
            doc.font('Courier').fontSize(10).fillColor('#555').text(headerLine2, margins.left, margins.top + 26, { width: width - margins.left - margins.right, align: 'left', lineBreak: false });
            // Footer (single line) at 30px from bottom
            doc.font('Courier').fontSize(10).fillColor('#555').text(footerText, margins.left, height - 50, { width: width - margins.left - margins.right, align: 'right', lineBreak: false });
        };
        ...
        const drawArcTextCentered = (doc, text, cx, cy, radius, centerAngleRad, arcSpanRad) => {
        ...
        const drawStamp = (doc, centerX, centerY) => {
        ...
        doc.font(labelFont).fontSize(11).fillColor('#444').text('This certificate verifies authorship and issuance for the submitted work.', labelX, y + lineHeight, { width: doc.page.width - labelX - 70, align: 'left' });
        ...
        doc.y = CONTENT_START_Y;
        doc.font('Courier-Bold').fontSize(14).fillColor('#111').text('Submitted Work', margins().left, doc.y, {
            underline: true,
        ...
        const publicDir = path_1.default.join(__dirname, '..', '..', 'public', 'licenses', license.userId);
        fs_1.default.mkdirSync(publicDir, { recursive: true });
        const finalPath = path_1.default.join(publicDir, `${license.id}.pdf`);
        ...
        await licenseService_1.licenseService.updateLicenseUrl(license.id, publicUrl);
