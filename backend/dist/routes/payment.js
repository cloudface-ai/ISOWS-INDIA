...
router.get('/plans', async (req, res) => {
    try {
        const plans = await paymentService_1.paymentService.getPlans();
        res.json({ plans });
    }
    catch (error) {
...
router.post('/create-order', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const order = await paymentService_1.paymentService.createOrder(planId, userId, userEmail, userName);
        res.json({
            orderId: order.id,
            amount: order.amount,
...
router.post('/create-subscription', auth_1.authMiddleware, async (req, res) => {
    try {
        const { planId } = req.body;
        const userId = req.user?.uid;
        const userEmail = req.user?.email;
        ...
        const subscription = await paymentService_1.paymentService.createSubscription(planId, userId, userEmail, userName);
        res.json({
...
// Verify payment
router.post('/verify-payment', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const verification = await paymentService_1.paymentService.verifyPayment(paymentId, orderId, signature);
...
router.get('/subscriptions', auth_1.authMiddleware, async (req, res) => {
    try {
        const userId = req.user?.uid;
