import { PlagiarismResult, Work } from '../types'; 
import { workService } from './workService';
import { emailService } from './emailService';

export const plagiarismService = {
  async checkPlagiarism(content: string, currentUserId?: string): Promise<PlagiarismResult> {
    ...
    // Build n-grams (n=3)
    const makeNgrams = (arr: string[], n: number) => {
      const out: string[] = [];
    ...
    const allWorks: Work[] = await workService.getAllWorks();
    ...
    for (const w of otherWorks) {
      ...
      for (const tri of trigrams) {
        if (otherTrigrams.has(tri)) { overlap++; overlappingPhrases.push(tri); }
      }
  ...
  async checkPlagiarismWithNotification(
    content: string, 
    workId: string, 
    userId: string, 
    userEmail: string, 
    ...
    if (result.isPlagiarized && result.matches && result.matches.length > 0) {
      try {
        ...
        const similarWorks = await workService.getAllWorks();
        const matchingWorks = similarWorks.filter(w => similarWorkIds.includes(w.id));
        
        await emailService.sendPlagiarismAlert(
          userEmail,
          workTitle,
          workId,
          result,
